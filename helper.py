...
class FeatureTransform(object):
    ...
    * interaction is bool: 
    ...
    def ftr_inter_mat(self):
        """
        Feature transformation function of polynomial w/ interaction term:
        output is matrix of feature transformation of 
            X w/ dim of K+1 by N (np.ndarray);
        """
        self.degree = 2
...
class Regression(object):
    ...
    def Run(self, method):
        ...
        if method == 'LS':
            ...
            return theta_mu, theta_sigma,f_mu,f_sigma,MSE
        else:
            print('No such method!!!!')
            
    def Plot(self, method, path):
        """
        """
        if method == 'LS' or \
        method == 'RLS' or \
        method == 'LASSO' or \
        method == 'RR':
            [theta,f,MSE] = self.Run(method)
            plt.plot(self.trueX,self.trueY,c='r', label='true value')
            plt.scatter(self.X,self.Y,c='g', s = 10, label = 'sample')
            plt.plot(self.trueX, f ,c='b', 
                     marker = '.', 
                     linestyle = ':', 
                     label = 'estimated_'+str(method))
            plt.xlabel('x', fontsize=18)
            plt.ylabel('y', fontsize=18)
            plt.legend(loc = 'best', fontsize=18)
            fig = plt.gcf()
            fig.set_size_inches(18.5, 10.5)
            plt.savefig(os.path.join(path,'plot_'+str(method)+".png"), dpi = 100)
            plt.show()
        elif method == 'BR':
            [theta_mu, theta_sigma,f_mu,f_sigma,MSE] = self.Run(method)
            plt.plot(self.trueX, self.trueY, c='r', label='true value')
            plt.scatter(self.X, self.Y, c='g', s = 10, label = 'sample')
            plt.plot(self.trueX,f_mu,c='b', 
                     marker = '.', 
                     label = 'estimated_'+str(method))
            plt.fill_between(self.trueX,
                             f_mu-np.diag(f_sigma), 
                             f_mu+np.diag(f_sigma), 
                             label = '['+r'$\hat{\mu}^{*}-\hat{\sigma}_{*}$' +
                             ', '+
                             r'$\hat{\mu}^{*}+\hat{\sigma}_{*}$'+']')
            plt.xlabel('x', fontsize=18)
            plt.ylabel('y', fontsize=18)
            plt.legend(loc = 'best', fontsize=18)
            fig = plt.gcf()
